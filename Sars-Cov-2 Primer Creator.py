...
#SARS-CoV-2 (Covid-19): NC_045512


# Contains 9 DNA strands from the coronavirus family
arr = [""]*9
strippedArr = [""]*9
seqID = "seqAlign-9.fasta"
ignore = 0

# # Contains 59 DNA strands from the coronavirus family
# arr = [""]*59
# strippedArr = [""]*59
# seqID = "seqAlign-59.fasta"
# ignore = 23

# For array index
i = 0

# For creating primers
for seq_record in SeqIO.parse(seqID, "fasta"):
    ...
    for currNuc in range(len(intSeq)):
        ...
        if findBlank(currNuc):
            ...
            if tracking:
                ...
                dist = currNuc - begin
                # patchwork fix
                if intSeq[begin] != '-' and intSeq[currNuc] != '-':
...
for (start,end) in s:
    ...
    while (abs(getTmScore(fPrimer) - getTmScore(rPrimer)) > 10):
        (newStart, sShift) = shiftStart(newStart, 18, newEnd - 18)
        (newEnd, eShift) = shiftEnd(newEnd, 18, newStart + 18)
        if(newStart == -1 or newEnd == -1 or (compStart == newStart and compEnd == newEnd)):
            skipFlag = True
            break 
        fPrimer = forwardPrimer(newStart, newStart + sShift)
        rPrimer = reversePrimer(newEnd - eShift, newEnd)
